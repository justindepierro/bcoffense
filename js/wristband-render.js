...
function getWristbandDisplayOptions() {
  return {
    ...
    componentNoVowels: [
      document.getElementById("wbNoVowelsFormation")?.checked && "formation",
      document.getElementById("wbNoVowelsFormation")?.checked && "form-tag-1",
      document.getElementById("wbNoVowelsFormation")?.checked && "form-tag-2",
      document.getElementById("wbNoVowelsShift")?.checked && "shift",
      document.getElementById("wbNoVowelsMotion")?.checked && "motion",
      document.getElementById("wbNoVowelsProtection")?.checked && "protection",
      document.getElementById("wbNoVowelsPlayTags")?.checked && "play-tag-1",
      document.getElementById("wbNoVowelsPlayTags")?.checked && "play-tag-2",
    ].filter(Boolean),
  ...
  if (renderPhoneEditor) {
    ...
    for (let index = 0; index < CELLS_PER_CARD; index += 1) {
      ...
      const sourceStatusBadge = play && typeof renderPlaySourceStatusBadge === "function"
        ...
        : "";
      const writeInHtml = renderWristbandCellWriteIn(custom, { forceStandalone: !play });
  ...
  for (let row = 0; row < WB_ROWS; row += 1) {
    ...
    if (oddPlay) {
      ...
      html += `<div class="wristband-cell" style="${oddStyle}" role="gridcell" tabindex="0"
                    ...
                    data-drag="wbCell" data-cell-idx="${oddIndex}"
                    data-card="${currentCardIndex}">${renderWristbandCellWriteIn(oddCustom, { forceStandalone: true })}</div>`;
    }
    ...
    if (evenPlay) {
      ...
      html += `<div class="wristband-cell" style="${evenStyle}" role="gridcell" tabindex="0"
                    ...
                    data-drag="wbCell" data-cell-idx="${evenIndex}"
                    data-card="${currentCardIndex}">${renderWristbandCellWriteIn(evenCustom, { forceStandalone: true })}</div>`;
    }
  }
