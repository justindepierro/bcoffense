...
function _csCardMarkup(card, side, editable = false) {
  ...
  return `<article class="cs-index-card${editable ? " cs-index-card--editor" : ""}"><div class="cs-index-card-head"><b>${escapeHtml(card?.name || "Game Day Card")}</b><span>${side === "front" ? "Front" : "Back"}</span></div><div class="cs-index-grid">${buckets.map((bucket) => _csIndexBucketMarkup(bucket, editable)).join("") || `<div class="cs-index-empty">Use + Add bucket to build this side.</div>`}</div></article>`;
}
