...
function _csIndexBucketMarkup(bucket, editable) {
  ...
  const plays = rows.map((row) => {
    ...
    previous = row.play;
    // Mirror the grid's per-cell formatting (colors, highlight, bold, etc.);
    // the index-card view previously dropped all of it.
    const cellStyle = typeof csBuildPlayCellStyle === "function"
      ? csBuildPlayCellStyle(row.play, {})
      : { styleStr: "", highlightClass: "" };
    const styleAttr = cellStyle.styleStr ? ` style="${cellStyle.styleStr}"` : "";
...
async function openCallSheetIndexCardBucketPicker(id) {
  ...
  if (!bucket.categoryId) {
    const categoryId = await showListPicker("Choose the Call Sheet situation to draw plays from. Your custom bucket keeps its own title and only includes the plays you add.", CALLSHEET_CATEGORIES.map((category) => ({
      ...
      sublabel: `${_csSafeList(callSheet?.[category.id]?.left).length + _csSafeList(callSheet?.[category.id]?.right).length} calls`,
    })), { title: "Add plays to custom bucket", icon: "＋" });
    if (!categoryId) return;
    const hash = await showListPicker("Which Call Sheet column should this custom bucket draw from?", [
    ...
    bucket.hash = hash;
    bucket.targetHash = hash;
    bucket.playKeys = Array.isArray(bucket.playKeys) ? bucket.playKeys : [];
...
async function _csAddBucket() {
  ...
  _csActiveCard()[_csIndexSide].push({ id: `bucket-${Date.now()}-${Math.random().toString(36).slice(2, 5)}`, label: label.trim(), categoryId, hash, targetHash: hash === "right" ? "right" : "left", family: {}, ...(categoryId ? {} : { playKeys: [], excludedPlayKeys: [] }) });
  _csPersistCards();
}
...
// This toolbar is rebuilt by every Call Sheet render, so its event guard must
// be installed once at script load rather than attached to a rendered element.
document.addEventListener("click", (event) => {
  const trigger = event.target instanceof Element
    ? event.target.closest("#csIndexToolbarContext .cs-index-main-more-trigger")
    : null;
  if (!trigger) return;
  // Consume in capture phase: otherwise the later global menu router can
  // immediately apply a second toggle and leave the menu visually closed.
  event.preventDefault();
