    ...
    [
      ...
      if (pos && pos.trim()) {
        ...
        if (playerName && playerName.trim()) {
          if (!entry.keyPlayers.has(posKey)) entry.keyPlayers.set(posKey, new Set());
...
function obBuildConceptMap() {
  ...
  plays.forEach((p) => {
    ...
    if (!entry) {
      entry = { name: base, plays: [], types: new Set(), constraints: new Set(), deadVs: new Set(), playNames: new Set() };
      map.set(base, entry);
    }
    entry.plays.push(p);
    if (p.type) entry.types.add(p.type);
    if (p.play) entry.playNames.add(p.play.trim());
    [p.constraint1, p.constraint2, p.constraint3].filter(Boolean).forEach((c) => entry.constraints.add(c.trim()));
    if (p.deadVs) entry.deadVs.add(p.deadVs.trim());
  });
...
function obAnalyzeGaps(playMap, conceptMap, ratings) {
  ...
  const ratedPlayNames = new Set(
    Object.entries(ratings).filter(([, r]) => r >= 1).map(([n]) => n)
  );
  ...
  neededConstraints.forEach((cLow) => {
    const found = Array.from(conceptMap.keys()).some((k) => k.toLowerCase() === cLow);
    if (!found) {
      recs.push({
        type: "missing-constraint",
        severity: "critical",
        concept: constraintSources[cLow] ? constraintSources[cLow].join(", ") : "",
        message: "Constraint \"" + cLow + "\" referenced but not found as a base concept",
  ...
  ratedPlayNames.forEach((playName) => {
    ...
    entry.keyPlayers.forEach((names, pos) => {
      if (names.size === 1) {
        ...
        recs.push({
          ...
          severity: "info",
          concept: playName,
          message: "\"" + playName + "\" depends on one player at " + pos + ": " + pName,
    ...
    if (rating >= 4 && entry.formations.size <= 1) {
      recs.push({
        ...
        severity: "info",
        concept: playName,
        message: "\"" + playName + "\" is rated " + rating + " stars but only runs from " + entry.formations.size + " formation",
  ...
  ratedPlayNames.forEach((playName) => {
    ...
    entry.types.forEach((t) => {
      ...
      if (["drop", "play action", "play pass", "quick", "screen", "movement"].includes(tl)) passCount++;
    });
  });
  ...
  if (total > 0) {
    ...
    if (runPct > 70) {
      recs.push({ type: "balance", severity: "warn", message: "Your rated plays skew heavily run (" + runPct + "%). Consider adding more passing plays." });
    } else if (runPct < 30) {
      recs.push({ type: "balance", severity: "warn", message: "Your rated plays skew heavily pass (" + (100 - runPct) + "%). Consider adding more run plays." });
    }
  }
...
function obGetFilteredPlays() {
  ...
  if (obFilterType === "run") {
    ...
    entries = entries.filter((e) =>
      Array.from(e.types).some((t) => ["Drop", "Play Action", "Play Pass", "Quick", "Screen", "Movement"].includes(t))
    );
  }
    ...
    '<div class="ob-container">' +
      '<div class="ob-header">' +
        ...
        '<div class="ob-header-right">' +
          '<div class="ob-stat"><span class="ob-stat-num" id="obStatTotal">' + totalPlays + '</span><span class="ob-stat-label">Plays</span></div>' +
          '<div class="ob-stat"><span class="ob-stat-num" id="obStatRated">' + ratedCount + '</span><span class="ob-stat-label">Rated</span></div>' +
          '<div class="ob-stat ob-stat-alert"><span class="ob-stat-num" id="obStatGaps">' + criticalRecs.length + '</span><span class="ob-stat-label">Gaps</span></div>' +
        '</div>' +
      '</div>' +
      '<div class="ob-toolbar">' +
        '<input type="text" class="ob-search" id="obSearchInput" placeholder="Search plays, concepts, formations..." value="' + obEscapeAttr(obSearchTerm) + '">' +
        '<select class="ob-filter-select" id="obFilterType">' +
          '<option value=""' + (obFilterType === "" ? " selected" : "") + '>All Types</option>' +
          '<option value="run"' + (obFilterType === "run" ? " selected" : "") + '>\u{1F3C3} Run Game</option>' +
          '<option value="pass"' + (obFilterType === "pass" ? " selected" : "") + '>\u{1F3AF} Pass Game</option>' +
        '</select>' +
        '<label class="ob-toggle-label">' +
          '<input type="checkbox" id="obShowRated" ' + (obShowRatedOnly ? "checked" : "") + '>' +
...
function obRenderPlayList() {
  ...
  if (entries.length === 0) {
    listEl.innerHTML = '<div class="ob-empty-list">No plays match your filters</div>';
    return;
  }
  ...
  for (var i = 0; i < entries.length; i++) {
    ...
    var hasGap = entry.constraints.size > 0 &&
      Array.from(entry.constraints).some(function (c) {
        var cLow = c.toLowerCase();
        return !Array.from(_obConceptMap.keys()).some(function (k) { return k.toLowerCase() === cLow; });
      });

    html += '<div class="ob-card' + (isActive ? " ob-card-active" : "") + (rating >= 4 ? " ob-card-core" : "") + '" data-play="' + obEscapeAttr(entry.name) + '">';
    html += '<div class="ob-card-top">';
    html += '<div class="ob-card-name">' + escapeHtml(entry.name) + '</div>';
    html += '<div class="ob-card-stars" data-star-target="' + obEscapeAttr(entry.name) + '">';
    ...
    html += '<span class="ob-meta-tag ob-meta-type">' + escapeHtml(typeLabels) + '</span>';
    html += '<span class="ob-meta-tag">\u{1F4D0} ' + formationCount + '</span>';
    if (rowCount > 1) html += '<span class="ob-meta-tag">\u{1F4CB} ' + rowCount + ' rows</span>';
    if (entry.constraints.size > 0) html += '<span class="ob-meta-tag ob-meta-constraint">\u{1F517} ' + entry.constraints.size + '</span>';
    if (hasGap) html += '<span class="ob-meta-tag ob-meta-gap">\u26A0\uFE0F Gap</span>';
    html += '</div></div>';
  }
...
function obRenderStarPicker(playName, current) {
  ...
  for (var i = 1; i <= 5; i++) {
    ...
    html += '<span class="ob-star ' + (filled ? "ob-star-filled" : "ob-star-empty") + '" data-value="' + i + '" title="' + i + ' star' + (i > 1 ? "s" : "") + '">\u2605</span>';
  }
  if (current > 0) {
...
function _obUpdateHeaderStats() {
  var ratings = obLoadRatings();
  var ratedCount = Object.values(ratings).filter(function (r) { return r >= 1; }).length;
  var recs = obAnalyzeGaps(_obPlayMap, _obConceptMap, ratings);
  var criticalRecs = recs.filter(function (r) { return r.severity === "critical"; });
...
function obRenderSidebar() {
  ...
  sidebarEl.innerHTML = _obBuildDetailHtml() + _obBuildRecommendationsHtml(recs);
...
function _obBuildDetailHtml() {
  ...
  // Installation status
  var installData = typeof getInstallationData === "function" ? getInstallationData() : { installed: {} };
  var installedPlays = (installData.installed && installData.installed.play) || [];
  var installedSet = new Set(installedPlays.map(function (v) { return v.trim(); }));

  // Constraints
  var constraintParts = [];
  if (entry.constraints.size > 0) {
    entry.constraints.forEach(function (c) {
      var exists = _obConceptMap && Array.from(_obConceptMap.keys()).some(function (k) { return k.toLowerCase() === c.toLowerCase(); });
      constraintParts.push(
        '<span class="ob-constraint-chip ' + (exists ? "ob-constraint-found" : "ob-constraint-missing") + '"' +
        (exists ? ' data-concept="' + obEscapeAttr(c) + '"' : '') +
        ' title="' + (exists ? "Click to explore" : "Not in playbook") + '">' +
        escapeHtml(c) + ' ' + (exists ? "\u2705" : "\u26A0\uFE0F") + '</span>'
      );
    });
  }
  var constraintHtml = constraintParts.length > 0 ? constraintParts.join("") : '<span class="ob-no-data">None listed</span>';

  // Dead vs
  var deadVsParts = [];
  entry.deadVs.forEach(function (d) { deadVsParts.push('<span class="ob-dead-chip">\u{1F6AB} ' + escapeHtml(d) + '</span>'); });
  var deadVsHtml = deadVsParts.length > 0 ? deadVsParts.join("") : '<span class="ob-no-data">None listed</span>';
  ...
  // Hit chart
  var hitChartEntries = Object.entries(entry.hitCharts).sort(function (a, b) { return b[1] - a[1]; });
  var hitChartHtml = hitChartEntries.length > 0
    ? hitChartEntries.map(function (pair) { return '<span class="ob-hc-chip">' + escapeHtml(pair[0]) + ' <small>(' + pair[1] + ')</small></span>'; }).join("")
    : '<span class="ob-no-data">None</span>';

  // Formations
  var formationsHtml = Array.from(entry.formations).sort().map(function (f) {
    return '<span class="ob-form-chip">' + escapeHtml(f) + '</span>';
  }).join("") || '<span class="ob-no-data">\u2014</span>';

  // Personnel
  var personnelHtml = Array.from(entry.personnel).map(function (p) {
    return '<span class="ob-pers-chip">' + getPersonnelEmoji(p) + ' ' + escapeHtml(p) + '</span>';
  }).join("") || '<span class="ob-no-data">\u2014</span>';

  // Base concepts
  var basePlaysHtml = Array.from(entry.basePlays).map(function (bp) {
    return '<span class="ob-form-chip">' + escapeHtml(bp) + '</span>';
  }).join("") || '<span class="ob-no-data">\u2014</span>';

  // Variations table
  var rowHtml = entry.rows.map(function (p) {
  ...
  if (entry.basePlays.size > 0 && _obPlayMap) {
    ...
    _obPlayMap.forEach(function (otherEntry, otherName) {
      ...
      var shared = Array.from(otherEntry.basePlays).some(function (bp) { return entry.basePlays.has(bp); });
      if (shared) relatedNames.push(otherName);
    });
    relatedNames.sort();
    if (relatedNames.length > 0) {
      var relatedChips = relatedNames.slice(0, 20).map(function (rn) {
        var rRating = ratings[rn] || 0;
        return '<span class="ob-constraint-chip ob-constraint-found ob-related-chip" data-related-play="' + obEscapeAttr(rn) + '">' + escapeHtml(rn) + (rRating > 0 ? ' \u2605' + rRating : '') + '</span>';
      }).join("");
      relatedHtml =
        '<div class="ob-detail-section">' +
          '<div class="ob-section-title">\u{1F500} Related Plays (same base concept)</div>' +
          '<div class="ob-section-body">' + relatedChips +
            (relatedNames.length > 20 ? '<span class="ob-no-data">+' + (relatedNames.length - 20) + ' more</span>' : '') +
          '</div>' +
        '</div>';
    }
  }
  ...
  // Notes
  var allNotes = entry.rows.map(function (p) { return (p.notes || "").trim(); }).filter(Boolean);
  var uniqueNotes = [];
  var notesSeen = {};
  allNotes.forEach(function (n) { if (!notesSeen[n]) { notesSeen[n] = true; uniqueNotes.push(n); } });
  var notesHtml = uniqueNotes.length > 0
    ? uniqueNotes.map(function (n) { return '<div class="ob-note-item">' + escapeHtml(n) + '</div>'; }).join("")
    : '<span class="ob-no-data">None</span>';

  return '<div class="ob-detail" id="obDetailPanel">' +
    '<div class="ob-detail-title">' +
      '<h3>' + escapeHtml(entry.name) + '</h3>' +
      '<div class="ob-detail-stars" data-star-target="' + obEscapeAttr(entry.name) + '">' +
        obRenderStarPicker(entry.name, rating) +
      '</div>' +
    '</div>' +
    '<div class="ob-detail-stats">' +
      '<span>' + entry.rows.length + ' variation' + (entry.rows.length > 1 ? 's' : '') + '</span>' +
      '<span>' + entry.formations.size + ' formation' + (entry.formations.size !== 1 ? 's' : '') + '</span>' +
      '<span>' + Array.from(entry.types).join(", ") + '</span>' +
    '</div>' +
    '<div class="ob-detail-section"><div class="ob-section-title">\u{1F4E6} Base Concept</div><div class="ob-section-body">' + basePlaysHtml + '</div></div>' +
    '<div class="ob-detail-section"><div class="ob-section-title">\u{1F517} Constraints & Counters</div><div class="ob-section-body">' + constraintHtml + '</div></div>' +
    '<div class="ob-detail-section"><div class="ob-section-title">\u{1F6AB} Dead Vs</div><div class="ob-section-body">' + deadVsHtml + '</div></div>' +
    '<div class="ob-detail-section"><div class="ob-section-title">\u{1F464} Jimmys & Joes</div><div class="ob-section-body ob-kp-list">' + keyPlayerHtml + '</div></div>' +
    '<div class="ob-detail-section"><div class="ob-section-title">\u{1F3AF} Hit Chart Areas</div><div class="ob-section-body">' + hitChartHtml + '</div></div>' +
    '<div class="ob-detail-section"><div class="ob-section-title">\u{1F465} Personnel</div><div class="ob-section-body">' + personnelHtml + '</div></div>' +
    '<div class="ob-detail-section"><div class="ob-section-title">\u{1F4D0} Formations</div><div class="ob-section-body">' + formationsHtml + '</div></div>' +
    relatedHtml +
    '<div class="ob-detail-section"><div class="ob-section-title">\u{1F4CB} Variations (' + entry.rows.length + ')</div>' +
      ...
      '</table></div>' +
    '</div>' +
    '<div class="ob-detail-section"><div class="ob-section-title">\u{1F4DD} Notes</div><div class="ob-section-body">' + notesHtml + '</div></div>' +
  '</div>';
}
...
function _obAttachDetailHandlers() {
  ...
  var conceptChips = panel.querySelectorAll(".ob-constraint-chip[data-concept]");
  for (var i = 0; i < conceptChips.length; i++) {
    ...
    conceptChips[i].addEventListener("click", function () {
      ...
      for (var entry of _obPlayMap) {
        ...
        var data = entry[1];
        if (Array.from(data.basePlays).some(function (bp) { return bp.toLowerCase() === concept.toLowerCase(); })) {
...
function _obBuildRecommendationsHtml(recs) {
  ...
  var sorted = recs.slice().sort(function (a, b) { return (severityOrder[a.severity] || 9) - (severityOrder[b.severity] || 9); });
  ...
  for (var i = 0; i < limit; i++) {
    var r = sorted[i];
    rows += '<div class="ob-rec-row ob-rec-' + r.severity + '">' +
  ...
  return '<div class="ob-recs">' +
    '<div class="ob-recs-title">\u{1F4A1} Recommendations <span class="ob-recs-count">' + recs.length + '</span></div>' +
    rows +
