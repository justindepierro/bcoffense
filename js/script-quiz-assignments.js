...
function _quizAssignmentPlayerStatus(assignment) { const recipient = assignment?.recipient || {}; if (recipient.completedAt) return { label: `${recipient.bestPercent}% complete`, tone: "done" }; if (assignment?.dueAt && Number(assignment.dueAt) * 1000 < Date.now()) return { label: "Past due", tone: "late" }; if (recipient.startedAt) return { label: "In progress · practice recorded", tone: "progress" }; return { label: _quizAssignmentDueLabel(assignment?.dueAt), tone: "new" }; }
...
function renderCoachQuizAssignmentsPanel() {
  ...
  return `<section class="coach-quiz-setup-section coach-quiz-homework"><div class="coach-quiz-section-head"><div><h3>Private homework</h3><span>${recipients.length ? `${completed}/${recipients.length} player assignments complete` : "Build a quiz now, then send it when your install is ready."}</span></div><div class="coach-quiz-homework__actions"><button type="button" class="btn btn-outline btn-sm" data-action="refreshQuizAssignments">Refresh</button><button type="button" class="btn btn-primary" data-action="openQuizAssignmentManager">+ Build homework</button></div></div>${assignments.length ? `<div class="coach-quiz-homework-report"><span><b>${published.length}</b><small>sent assignments</small></span><span><b>${drafts}</b><small>saved drafts</small></span><span><b>${completed}/${recipients.length}</b><small>complete</small></span><span><b>${started}</b><small>in progress</small></span><span class="${overdue ? "is-attention" : ""}"><b>${overdue}</b><small>past due</small></span></div>` : ""}<div class="coach-quiz-homework__grid">${assignments.length ? assignments.slice(0, 8).map((assignment) => { const assignmentRecipients = assignment.recipients || []; const done = assignmentRecipients.filter((recipient) => recipient.completedAt).length; const count = Number(assignment.items?.length || 0) + Number(assignment.customQuestions?.length || 0); const isDraft = assignment.status === "draft"; const late = !isDraft && assignment.dueAt && Number(assignment.dueAt) * 1000 < Date.now() && done < assignmentRecipients.length; return `<article class="coach-quiz-homework-card${late ? " is-late" : ""}${isDraft ? " is-draft" : ""}"><strong>${escapeHtml(assignment.title)}${isDraft ? " <em>Draft</em>" : ""}</strong><small>${escapeHtml(isDraft ? `${count} questions · not sent to players` : `${count} questions · ${done}/${assignmentRecipients.length} complete · ${_quizAssignmentDueLabel(assignment.dueAt)}`)}</small><div>${assignmentRecipients.slice(0, 5).map((recipient) => `<span class="coach-quiz-homework-card__player${recipient.completedAt ? " is-done" : ""}">${escapeHtml(recipient.name)}${recipient.completedAt ? " ✓" : ""}</span>`).join("")}${assignmentRecipients.length > 5 ? `<span class="coach-quiz-homework-card__player">+${assignmentRecipients.length - 5}</span>` : ""}</div><button type="button" class="btn btn-sm btn-outline" data-action="${isDraft ? "editQuizAssignmentDraft" : "openQuizAssignmentDetails"}" data-arg="${escapeAttr(assignment.id)}">${isDraft ? "Continue draft" : "Manage"}</button></article>`; }).join("") : `<div class="coach-quiz-empty">No homework is out yet. Build a quiz now and send it when the install is ready.</div>`}</div></section>`;
}
...
function _newQuizAssignmentDraft() { return { assignmentId: "", status: "draft", title: "", instructions: "", requiredScore: 0, dueAt: "", quizMode: "quick", positionKey: "", recipientIds: new Set(), playKeys: new Set(), search: "", sourceKind: "playbook", sourceId: "", questionTypes: new Set(["responsibility", "diagram", "signal", "call"]), customQuestions: [], selectedGroup: "", frozenItems: null, frozenSourceLabel: "", lastSavedAt: 0 }; }
function _quizAssignmentHasDraftContent(draft = _quizAssignmentState.draft) { return Boolean(draft && (String(draft.title || "").trim() || String(draft.instructions || "").trim() || draft.playKeys?.size || draft.frozenItems?.length || draft.customQuestions?.length || draft.recipientIds?.size)); }
function _quizAssignmentPayload(draft = _quizAssignmentState.draft) { const items = _quizAssignmentSourceItems(draft); return { assignmentId: draft?.assignmentId || "", title: draft?.title || "", instructions: draft?.instructions || "", requiredScore: draft?.requiredScore || 0, dueAt: draft?.dueAt || "", quizMode: draft?.quizMode || "quick", positionKey: draft?.positionKey || "", recipientIds: [...(draft?.recipientIds || [])], items, questionTypes: [...(draft?.questionTypes || [])], customQuestions: (draft?.customQuestions || []).map((question) => ({ prompt: question.prompt, options: question.options, correctIndex: question.correctIndex })), sourceKind: draft?.sourceKind || "playbook", sourceId: draft?.sourceId || "" }; }
function _quizAssignmentModalScrollTop() { return document.querySelector("#quizAssignmentOverlay .quiz-assignment-modal")?.scrollTop || 0; }
function _restoreQuizAssignmentModalScrollTop(top) { requestAnimationFrame(() => { const modal = document.querySelector("#quizAssignmentOverlay .quiz-assignment-modal"); if (modal) modal.scrollTop = top; }); }
function _scheduleQuizAssignmentDraftSave() { const draft = _quizAssignmentState.draft; if (!_quizAssignmentHasDraftContent(draft)) return; clearTimeout(_quizAssignmentState.autosaveTimer); _quizAssignmentState.autosaveTimer = setTimeout(() => saveQuizAssignmentDraft({ quiet: true }), 900); }
...
function _renderQuizAssignmentModal() {
  ...
  const previousScrollTop = _quizAssignmentModalScrollTop();
  ...
  </div><footer><span id="quizAssignmentReadyCount">${draft.assignmentId ? "Draft saved · " : "Not sent · "}${draft.recipientIds.size} players · ${sourceItems.length + sourceHealth.custom.valid} ready questions</span><div><button type="button" class="btn btn-outline" data-action="closeQuizAssignmentManager">Close</button><button type="button" class="btn btn-outline" data-action="saveQuizAssignmentDraft">Save draft</button><button type="button" class="btn btn-primary" data-action="createQuizAssignment"${_quizAssignmentCanSubmit(draft, sourceHealth) ? "" : " disabled"} title="${escapeAttr(sourceHealth.ready ? (draft.recipientIds.size ? (String(draft.title || "").trim() ? "" : "Add a homework title first.") : "Choose at least one recipient.") : sourceHealth.issues.join(" "))}">Send homework</button></div></footer></div>`;
...
function setQuizAssignmentSource(kind) { const draft = _quizAssignmentState.draft; if (!draft) return; draft.frozenItems = null; draft.frozenSourceLabel = ""; draft.sourceKind = ["playbook", "script", "gameplan"].includes(kind) ? kind : "playbook"; draft.sourceId = ""; _scheduleQuizAssignmentDraftSave(); _renderQuizAssignmentModal(); }
function clearQuizAssignmentFrozenSource() { const draft = _quizAssignmentState.draft; if (!draft) return; draft.frozenItems = null; draft.frozenSourceLabel = ""; draft.sourceKind = "playbook"; _scheduleQuizAssignmentDraftSave(); _renderQuizAssignmentModal(); }
function setQuizAssignmentSourceId(value) { if (_quizAssignmentState.draft) { _quizAssignmentState.draft.sourceId = value; _scheduleQuizAssignmentDraftSave(); _renderQuizAssignmentModal(); } }
function toggleQuizAssignmentRecipient(id) { const set = _quizAssignmentState.draft?.recipientIds; if (!set) return; set.has(id) ? set.delete(id) : set.add(id); _scheduleQuizAssignmentDraftSave(); _renderQuizAssignmentModal(); }
function toggleAllQuizAssignmentPlayers() { const draft = _quizAssignmentState.draft; if (!draft) return; const ids = _quizAssignmentState.players.map((player) => player.id); if (draft.recipientIds.size === ids.length) draft.recipientIds.clear(); else ids.forEach((id) => draft.recipientIds.add(id)); _scheduleQuizAssignmentDraftSave(); _renderQuizAssignmentModal(); }
function toggleQuizAssignmentRecipientGroup(key) { const draft = _quizAssignmentState.draft; const group = _quizAssignmentGroups().find((entry) => entry.key === key); if (!draft || !group) return; const selected = group.ids.every((id) => draft.recipientIds.has(id)); group.ids.forEach((id) => selected ? draft.recipientIds.delete(id) : draft.recipientIds.add(id)); _scheduleQuizAssignmentDraftSave(); _renderQuizAssignmentModal(); }
function toggleQuizAssignmentPlay(key) { const set = _quizAssignmentState.draft?.playKeys; if (!set) return; set.has(key) ? set.delete(key) : set.add(key); _scheduleQuizAssignmentDraftSave(); _renderQuizAssignmentModal(); }
function toggleQuizAssignmentQuestionType(type) { const types = _quizAssignmentState.draft?.questionTypes; if (!types) return; types.has(type) ? types.delete(type) : types.add(type); _scheduleQuizAssignmentDraftSave(); _renderQuizAssignmentModal(); }
function useQuizAssignmentReadyQuestionTypes() { const draft = _quizAssignmentState.draft; if (!draft) return; const health = getQuizAssignmentSourceHealth(draft); draft.questionTypes = new Set(health.readyTypes); _scheduleQuizAssignmentDraftSave(); _renderQuizAssignmentModal(); }
function addQuizAssignmentCustomQuestion() { const draft = _quizAssignmentState.draft; if (!draft) return; draft.customQuestions.push({ id: `custom-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, prompt: "", options: ["", "", "", ""], correctIndex: 0 }); _scheduleQuizAssignmentDraftSave(); _renderQuizAssignmentModal(); }
function removeQuizAssignmentCustomQuestion(id) { const draft = _quizAssignmentState.draft; if (!draft) return; draft.customQuestions = draft.customQuestions.filter((question) => question.id !== id); _scheduleQuizAssignmentDraftSave(); _renderQuizAssignmentModal(); }
function setQuizAssignmentCustomQuestionField(arg, value) { const [id, field, index] = String(arg || "").split(":"); const question = _quizAssignmentState.draft?.customQuestions.find((entry) => entry.id === id); if (!question) return; if (field === "prompt") question.prompt = value; else if (field === "option") question.options[Number(index)] = value; else if (field === "correct") question.correctIndex = Number(value); _scheduleQuizAssignmentDraftSave(); refreshQuizAssignmentSourceHealth(); }
...
function applyQuizAssignmentTemplate(id) { const draft = _quizAssignmentState.draft; const template = _getQuizAssignmentTemplates().find((entry) => String(entry.id) === String(id)); if (!draft || !template) return; _applyQuizAssignmentTemplate(draft, template); _scheduleQuizAssignmentDraftSave(); _renderQuizAssignmentModal(); }
...
function _quizAssignmentRecipientLabel(recipient) { if (recipient.completedAt) return `${recipient.bestPercent}% complete`; if (recipient.startedAt) return "In progress · practice recorded"; return recipient.lastRemindedAt ? "Reminder sent" : "Not started"; }
function _quizAssignmentSeed(assignment, mode = "duplicate") {
  const draft = _newQuizAssignmentDraft();
  const editingDraft = mode === "edit";
  draft.assignmentId = editingDraft ? assignment.id : "";
  draft.status = editingDraft ? "draft" : "draft";
  draft.title = editingDraft ? assignment.title : mode === "followup" ? `Follow-up — ${assignment.title}` : `Copy — ${assignment.title}`;
  ...
  draft.frozenSourceLabel = editingDraft ? "Saved homework draft" : `${mode === "followup" ? "Follow-up from" : "Copy of"} ${assignment.title}`;
  (assignment.recipients || []).filter((recipient) => editingDraft || mode !== "followup" || !recipient.completedAt).forEach((recipient) => draft.recipientIds.add(recipient.userId));
  return draft;
}
function _renderQuizAssignmentDetails(assignment) {
  ...
  const footer = isDraft
    ? `<footer><span>This is saved only for coaches. Players cannot see it yet.</span><div><button type="button" class="btn btn-primary" data-action="editQuizAssignmentDraft" data-arg="${escapeAttr(assignment.id)}">Continue draft</button><button type="button" class="btn btn-danger" data-action="archiveQuizAssignment" data-arg="${escapeAttr(assignment.id)}">Discard</button></div></footer>`
...
runWhenDomReady(() => { setTimeout(() => refreshQuizAssignments({ quiet: true }), 600); });
