...
assert.match(storage, /function findPlayOnWristband\(play\)[\s\S]*?playbookId[\s\S]*?sourcePlayId[\s\S]*?originalPlayId[\s\S]*?wristbandLinkId[\s\S]*?candidate\?\.id[\s\S]*?wristbandIds\.has\(id\)[\s\S]*?playsMatch/, "wristband lookup matches on the durable play id (including the `id` field, where wristband cells store it) before any display-text fallback");
