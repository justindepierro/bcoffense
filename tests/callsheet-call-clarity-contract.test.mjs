...
assert.match(indexCards, /function _csBindIndexCardMoreMenuTrigger\(\)[\s\S]*?stopImmediatePropagation\(\)[\s\S]*?wrap\.classList\.toggle\("open", willOpen\)/, "the re-rendered Index Card overflow trigger has a direct one-button binding that cannot double-toggle through the global router");
