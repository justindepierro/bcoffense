...
- `CORE`: formation, play name, base play; personnel stays in play identity but
...
- [x] Mark personnel as a non-video cue because personnel is yelled or shown
  on picture boards, so missing personnel clips do not count against signal
...
- [x] Add future quiz hooks so players can be asked to identify the signal for a
  formation, motion, tag, or play name, while personnel remains a verbal/board
  cue.
...
- [x] Build a signal-only quiz item source from published signal clips, grouped
  by `CORE`, `TAGS`, `BLOCKING`, and `MOTIONS`, excluding non-video personnel
  cues.
...
- [x] Speed pass for Signal quizzes: correct answers should auto-advance to the
...
- [x] Add a future `Full Play Call` signal game: pull a play from the practice
  ...
  different play, missing motion, missing shift, or near-match tags; personnel
  can be shown as the normal board cue instead of requiring a personnel signal
  video.
