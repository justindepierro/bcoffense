...
<button
  type="button"
  class="btn btn-xs"
  data-action="resetColumnVisibility"
...
<button
  type="button"
  class="btn btn-xs pb-col-reset-btn"
  data-action="resetColumnVisibility"
>
  Reset Columns
</button>
...
<!-- before -->
<div
  class="pb-controls-row pb-chip-row pb-vision-only"
  id="pbChipsPictureRow"
  style="display:none"
>
  <!-- after -->
  <div
...
/* playbook.css */
.pb-picture-pill.pb-pic-wz {
  background: var(--pb-pic-wz-bg);
}
.pb-picture-pill.pb-pic-pull {
  background: var(--pb-pic-pull-bg);
}
.pb-picture-pill.pb-pic-dh {
  background: var(--pb-pic-dh-bg);
}
.pb-picture-pill.pb-pic-af {
  background: var(--pb-pic-af-bg);
}
```
...
<button
  ...
  aria-label="Sort primary field: ascending"
>
  &#9650;
</button>
```
...
```js
if (primaryDir) {
  primaryDir.setAttribute(
    "aria-label",
    `Sort primary field: ${currentSortDirection === "asc" ? "ascending" : "descending"}`,
  );
}
```
...
<button
  ...
  data-action="toggleParentOpen"
  title="Playbook analytics..."
  aria-label="Open analytics reports menu"
  aria-haspopup="true"
>
  📊 Analytics ▾
</button>
...
<th scope="col" class="col-gameplan" title="Game Plan" aria-label="Game Plan">
  🎯
</th>
<th scope="col" class="col-install" data-action="sortPlaybook" ...>
  ★ <span ...></span>
</th>
...
<div class="pb-drawer-filter-row">
  ...
  <select
    ...
    data-onchange="highlightWristbandPlays"
    ...
  >
...
const moreCount = moreFilterIds.filter(
...
if (toggle) {
  ...
  if (!badge) {
    ...
    badge.className = "pb-more-count badge badge-primary";
...
<input
  ...
  id="searchPlay"
  placeholder="Search plays…"
  aria-label="Search plays (press / to focus)"
  ...
/>
```
...
```html
<button
  class="btn btn-xs pb-shortcuts-hint"
  data-action="showKeyboardShortcuts"
...
if (
  _statsBarCache &&
  !activeTypeChips?.size &&
  !activePersonnelChips?.size &&
  !activePictureChips?.size
) {
...
_Audit completed 2026-06-24. All 28 items shipped. SW v675 / commit `ab824fb`._
