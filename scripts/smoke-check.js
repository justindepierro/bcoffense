...
function checkPageStyleContracts() {
  ...
  if (
    ...
    !/\.gp-cmd-actions\s*\{[\s\S]*display:\s*flex[\s\S]*align-items:\s*end/.test(gameplanCss) ||
    !/\.gp-command-group-actions\s*\{[\s\S]*display:\s*grid[\s\S]*grid-template-columns:\s*repeat\(2,\s*minmax\(86px,\s*max-content\)\)/.test(gameplanCss) ||
    !/\.gp-command-group \.btn\s*\{[\s\S]*min-width:\s*0[\s\S]*white-space:\s*normal[\s\S]*overflow-wrap:\s*anywhere/.test(gameplanCss) ||
    !/@media \(max-width:\s*1180px\)[\s\S]*#gameplan \.gp-cmd-main\s*\{[\s\S]*grid-template-columns:\s*1fr[\s\S]*#gameplan \.gp-cmd-actions\s*\{[\s\S]*flex-wrap:\s*wrap/.test(gameplanCss)
  ) {
